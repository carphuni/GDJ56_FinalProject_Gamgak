...
function selectFriendList(data, loginMemberNo){
	data.list.forEach(v => {
		...
		const $btChat=$("<button>").attr("onclick","chatStartx()");
		const $btA=$("<button>");
	...
	data.forEach(v => {
		...
		if(v.MEMBER_NO==loginMemberNo){
			...
			//로그인 회원이 친구신청을 했다면 대기버튼
				...
				$btChat.attr("class","cancleF btn btn-danger");
				$btChat.text("신청대기");
				$divFC.append($btChat);		
			//로그인 회원이 친구신청을 받았다면 수락버튼	
			}else if(v.MEMBER_FOLLOWER_NO==loginMemberNo&&v.ACCEPT_YN=='S'){
				$btChat.attr("class","acceptF btn btn-danger");
				$btChat.text("친구수락");
				$divFC.append($btChat);					
			//아무 관계가 아니라면 추가버튼
			}else{
				$btChat.attr("class","insertFriend btn btn-danger");
				$btChat.text("친구추가");
				$divFC.append($btChat);			
			}
			$divFC.append($inputH);
